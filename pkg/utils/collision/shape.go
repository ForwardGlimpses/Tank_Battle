...
type collidingLine struct {
	Start, End vector2.Vector
}

func new_line(x, y, x2, y2 float64) *collidingLine {
	return &collidingLine{
		Start: vector2.NewVector(x, y),
		End:   vector2.NewVector(x2, y2),
	}
}

func (line *collidingLine) Project(axis vector2.Vector) vector2.Vector {
	return line.Vector().MulScale(axis.Dot(line.Start.Sub(line.End)))
}

func (line *collidingLine) Normal() vector2.Vector {
	v := line.Vector()
	return vector2.NewVector(v.Y, -v.X).Unit()
}

func (line *collidingLine) Vector() vector2.Vector {
	return line.End.Sub(line.Start).Unit()
}

// IntersectionPointsLine returns the intersection point of a Line with another Line as a vector2.Vector, and if the intersection was found.
func (line *collidingLine) IntersectionPointsLine(other *collidingLine) (vector2.Vector, bool) {
	...
	if det != 0 {
		...
		if (0 < lambda && lambda < 1) && (0 < gamma && gamma < 1) {
			...
			return vector2.NewVector(line.Start.X+(lambda*dx), line.Start.Y+(lambda*dy)), true
		}
...
func (line *collidingLine) IntersectionPointsCircle(circle *Circle) []vector2.Vector {
	...
	if det < 0 {
		...
		if t >= 0 && t <= 1 {
			points = append(points, vector2.NewVector(line.Start.X+t*diff.X, line.Start.Y+t*diff.Y))
		}
		...
		// We have to ensure t is between 0 and 1; otherwise, the inspector points are on the circle as though the lines were infinite in length.
		if t >= 0 && t <= 1 {
			points = append(points, vector2.NewVector(line.Start.X+t*diff.X, line.Start.Y+t*diff.Y))
		}
		t = (-b - math.Sqrt(det)) / (2 * a)
...
func (cp *ConvexPolygon) Clone() IShape {
	...
	points := append(make([]vector2.Vector, 0, len(cp.Points)), cp.Points...)
...
func (cp *ConvexPolygon) AddPoints(vertexPositions ...float64) {
	...
	for v := 0; v < len(vertexPositions); v += 2 {
		cp.Points = append(cp.Points, vector2.NewVector(vertexPositions[v], vertexPositions[v+1]))
	}
}
...
func (cp *ConvexPolygon) Transformed() []vector2.Vector {
	transformed := []vector2.Vector{}
	for _, point := range cp.Points {
		p := vector2.NewVector(point.X*cp.scale.X, point.Y*cp.scale.Y)
		if cp.rotation != 0 {
			p = p.Rotate(-cp.rotation)
		}
		transformed = append(transformed, vector2.NewVector(p.X+cp.position.X, p.Y+cp.position.Y))
	}
	return transformed
}
...
func (cp *ConvexPolygon) Bounds() (vector2.Vector, vector2.Vector) {
	...
	topLeft := vector2.NewVector(transformed[0].X, transformed[0].Y)
	bottomRight := topLeft
...
// SetPositionVec allows you to set the position of the ConvexPolygon using a vector2.Vector. The offset of the vertices compared to the X and Y
...
func NewContactSet() *ContactSet {
	return &ContactSet{
		Points: []vector2.Vector{},
		MTV:    vector2.NewVector(0, 0),
		Center: vector2.NewVector(0, 0),
	}
}
...
func (cp *ConvexPolygon) Intersection(dx, dy float64, other IShape) *ContactSet {
	...
	if contactSet != nil && (dx != 0 || dy != 0) {
		...
		deltaMagnitude := vector2.NewVector(dx, dy).Magnitude()
		contactSet.MTV = contactSet.MTV.Unit().MulScale(ogMagnitude - deltaMagnitude)
	}
...
func (cp *ConvexPolygon) calculateMTV(contactSet *ContactSet, otherShape IShape) (vector2.Vector, bool) {

	delta := vector2.NewVector(0, 0)

	smallest := vector2.NewVector(math.MaxFloat64, 0)
	...
	case *ConvexPolygon:
		...
		for _, axis := range cp.SATAxes() {
			...
			if smallest.Magnitude() > overlap {
				smallest = axis.MulScale(overlap)
			}
	...
	case *Circle:
		...
		verts := append([]vector2.Vector{}, cp.Transformed()...)
		...
		smallest = vector2.NewVector(center.X-verts[0].X, center.Y-verts[0].Y)
		smallest = smallest.Unit().MulScale(smallest.Magnitude() - other.radius)
...
func (cp *ConvexPolygon) FlipH() {
	...
	// Colliders (so that the normals are consistently outside or inside, which is important when doing Intersection tests).
	// If we assume that the normal of a line, going from vertex A to vertex B, is one direction, then the normal would be
	// inverted if the vertices were flipped in position, but not in order. This would make Intersection tests drive colliders
...
func (cp *ConvexPolygon) RecenterPoints() {
	...
	offset := vector2.NewVector(0, 0)
	for _, p := range cp.Points {
		// vector.In(offset).Add(p)
		offset = offset.Add(p)
	}

	offset = offset.MulScale(1.0 / float64(len(cp.Points))).Invert()
...
func (cp *ConvexPolygon) ReverseVertexOrder() {
	...
	verts := []vector2.Vector{cp.Points[0]}
...
func (circle *Circle) Bounds() (vector2.Vector, vector2.Vector) {
	return vector2.NewVector(circle.position.X-circle.radius, circle.position.Y-circle.radius), vector2.NewVector(circle.position.X+circle.radius, circle.position.Y+circle.radius)
}
...
func (circle *Circle) Intersection(dx, dy float64, other IShape) *ContactSet {
	...
	case *ConvexPolygon:
		...
		if contactSet != nil {
			contactSet.MTV = contactSet.MTV.MulScale(-1)
		}
	...
	case *Circle:
		...
		contactSet.MTV = vector2.NewVector(circle.position.X-shape.position.X, circle.position.Y-shape.position.Y)
		dist := contactSet.MTV.Magnitude()
		contactSet.MTV = contactSet.MTV.Unit().MulScale(circle.radius + shape.radius - dist)
		...
		// contactSet.MTV = vector2.NewVector(circle.X - shape.X, circle.Y - shape.Y}
	}
...
func (circle *Circle) IntersectionPointsCircle(other *Circle) []vector2.Vector {
	...
	return []vector2.Vector{
		{X: x2 + h*(other.position.Y-circle.position.Y)/d, Y: y2 - h*(other.position.X-circle.position.X)/d},
		{X: x2 - h*(other.position.Y-circle.position.Y)/d, Y: y2 + h*(other.position.X-circle.position.X)/d},
	}
...
func (circle *Circle) Scale() vector2.Vector {
	return vector2.NewVector(circle.scale, circle.scale)
}
...
// func (ms *MultiShape) Bounds() (vector2.Vector, vector2.Vector) {

// 	if len(ms.Shapes) == 0 {
// 		return vector2.NewVector(}, vector2.NewVector(}
// 	}
